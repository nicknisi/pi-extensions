...
| [lsp](packages/lsp/)                 | Read-only TypeScript/JavaScript definitions, references, hover, symbols, and diagnostics                                                                                                                                                                                      | `lsp`                                        |
...
| [workflows](packages/workflows/)     | Model-facing front door to the first-party workflow engine: run JS workflow scripts over the subagent runtime with the old third-party tool's globals (`agent`, `parallel`, `pipeline`, `phase`, `log`, `args`, `budget`, `cwd`) — evicts `@quintinshaw/pi-dynamic-workflows` | `workflow` tool, `/wf`                       |
...
| [checkpoint](packages/checkpoint/)       | Durable self-compaction: a note-bearing `self_compact` tool checkpoints a long-running agent and resumes its exact next action after compaction, with threshold controls and a context widget | `self_compact` tool, `/self-compact-info`, `/self-compact-now` |
