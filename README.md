...
| [self-compact](packages/self-compact/)   | Durable self-compaction: a note-bearing `self_compact` tool checkpoints a long-running agent and resumes its exact next action after compaction, with threshold controls and a context widget | `self_compact` tool, `/self-compact-info`, `/self-compact-now` |
