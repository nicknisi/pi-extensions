...
| [codemode](packages/codemode/)       | Model-written TypeScript orchestrates subagents compositionally (Promise.all fan-out, pipelines) — executed in-process, returns the module result; `=` console prefix runs the same runtime inline | `codemode` tool, `=` prefix                  |
