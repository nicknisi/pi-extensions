...
export default function codemode(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: 'Codemode',
    description: [
      'Execute a TypeScript snippet that orchestrates subagents and returns a result. Injected',
      'bindings: spawn(options), runWorkflow(spec, opts?), log(...args). No imports; the snippet is',
      'bundled standalone. It MUST `export default` a small summary (counts, findings, short lists;',
      'never raw file contents). Never busy-wait or run long synchronous loops: they block the host',
      'event loop and the timeout cannot preempt them. Wrap non-spawn steps that can throw in',
      'try/catch; an uncaught throw fails the run. Runs in the host process with full Node access,',
      'the same trust boundary as bash.',
      'spawn(options) launches a hermetic child session (no user extensions, skills, or context) and',
      'never rejects; check result.ok. Success: { ok: true, text, data?, usage, durationMs, runId }.',
      "Failure: { ok: false, kind: 'crashed'|'empty'|'schema_invalid'|'aborted', error, text, usage,",
      "durationMs, runId }. SpawnOptions: { prompt (required); agent?: label; model?: 'provider/id'",
      '(default configs/codemode.json childModel, else the session model); thinkingLevel? (default',
      'childThinkingLevel, else the session level); tools?: allowlist (default read-only [read, grep,',
      'find, ls]; pass [read, bash, edit, write] for builders); systemPrompt?; replaceSystemPrompt?;',
      'extensionPaths?; skillPaths?; includeContextFiles?; outputSchema?: JSON schema (validated,',
      "parsed into result.data; failure after one repair attempt returns kind 'schema_invalid');",
      'text?: true for raw text. Pass outputSchema or text:true; omitting both throws synchronously.',
      'cwd?; timeoutMs? (default 15 min); maxTurns?; maxToolCalls? }.',
      'Fan out independent work with Promise.all. For dependent stages, gates, retries, or edits to',
      'the working tree use runWorkflow(spec): { name, stages: [{ id, prompt (string or (ctx, item?,',
      'index?) => string; ctx = { results, treeDiffs, cwd, runDir }), needs?: string[] (default: the',
      'previous stage; pass [] to run independently), model?, tools?, systemPrompt?, outputSchema?,',
      'foreach?: unknown[] | { from: stageId, pick?: (outcome) => unknown[] }, gate?: (outcome, ctx)',
      '=> true | { revise: feedback }, maxGateAttempts?, retries?, sharesTree?: boolean (never overlaps',
      'other stages; its git diff HEAD flows to dependents via treeDiffs), maxTurns?, maxToolCalls?,',
      'timeoutMs? }], concurrency?,',
      'tokenBudget? }. Resolves to { ok, outcomes, usage, runDir }; never rejects.',
    ].join(' '),
